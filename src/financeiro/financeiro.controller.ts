...
export class FinanceiroController {
  ...
  listarCategorias(@Request() req: any) {
    ...
    return this.svc.listarCategorias(
      c.tipo === 'super_admin' ? null : c.bandaId,
    );
  }
  ...
  criarCategoria(@Body() dto: CreateCategoriaDto, @Request() req: any) {
    const c = this.caller(req);
    return this.svc.criarCategoria(
      dto,
      c.tipo === 'super_admin' ? null : c.bandaId,
    );
  }
  ...
  listarDespesas(
    ...
    const c = this.caller(req);
    return this.svc.listarDespesas(
      eventoId,
      c.tipo === 'super_admin' ? null : c.bandaId,
    );
  }
  ...
  listarPagamentos(
    ...
    const c = this.caller(req);
    return this.svc.listarPagamentos(
      eventoId,
      c.tipo === 'super_admin' ? null : c.bandaId,
    );
  }
  ...
  criarPagamento(@Body() dto: CreatePagamentoDto, @Request() req: any) {
    const c = this.caller(req);
    return this.svc.criarPagamento(
      dto,
      c.tipo === 'super_admin' ? null : c.bandaId,
    );
  }
